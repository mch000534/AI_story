...
#### F1. AI 設定管理
- **F1.1** 提供設定頁面，包含以下欄位：
  - [x] AI Base URL（支持 OpenAI相容自訂端點）
  - [x] API Key（加密儲存，支援顯示/隱藏切換）
  - [x] 模型選擇（自動獲取列表，並支援下拉選單與自訂輸入）
  - [x] Temperature（滑桿，範圍 0.0-2.0）
  - [x] Top-P（滑桿，範圍 0.0-1.0）
...
- **F2.4** 版本管理
  - [x] 保存每次生成的版本
  - [x] 可以比對不同版本 (並列 Diff 高亮)
  - [x] 恢復到歷史版本 (包含從比較頁面直接套用)
  - [x] 版本重命名與刪除功能
  - [x] 全屏預覽與比較介面
